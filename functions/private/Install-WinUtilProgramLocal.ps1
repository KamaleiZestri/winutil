function Install-WinUtilProgramLocal
{
    ...
    {
        $result = $false
        Set-WinUtilProgressBar -label "$Action $($program.Name)" -percent ($i / $count * 100)
        $sync.form.Dispatcher.Invoke([action]{ Set-WinUtilTaskbaritem -value ($i / $count)})

        $result = switch ($Action) {
            "Install" {Invoke-Install -program $program.Name -arguments $program.Value}
            "Uninstall" {Invoke-Uninstall -program $program.Name}
